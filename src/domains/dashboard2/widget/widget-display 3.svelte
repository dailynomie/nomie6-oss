...
<script lang="ts">
  ...
  let secondTrackable = $derived.by(() => {
    if (widget && widget.secondToken) {
      const resolved = tokenToTrackable(widget.secondToken, $TrackableStore.trackables)
      console.log('secondToken:', widget.secondToken, 'secondTrackable:', resolved)
      return resolved
    }
    return undefined
  })
    ...
    <div class="flex widget-header px-2 py-1">
      {#if trackable && widget.type != 'pointer'}
        <div class="flex items-center gap-1">
          ...
          {#if secondTrackable && ['barchart', 'linechart'].includes(widget.type)}
            {console.log('Showing second trackable pill')}
            <div class="flex items-center gap-1">
              <span style="color: {secondTrackable.color}; font-size: 0.9rem; font-weight: bold;">|</span>
              <TrackablePill hideValue size={30} transparent trackable={secondTrackable} on:click={widgetActions} />
            </div>
          {/if}
        </div>
