      ...
      {#if editingWidget?.type == "pointer"}
        {#if editingWidget?.data}
          ...
          <ListItem>
            ...
            <div slot="right" class="flex-1 pr-4">
              <input type="range" min="1" max="8" bind:value={editingWidget.data.pointersamples} step="1" class="pointer-slider w-full h-2">
            </div>
          </ListItem>
        {:else}
          <div class="text-gray-500 text-sm p-4">Initializing pointer widget...</div>
        {/if}
      {/if}
      <Divider left={18} />
      <ListItem>
...
<style lang="postcss">
  :global(.pointer-slider) {
    @apply appearance-none bg-transparent cursor-pointer h-2;
  }

  :global(.pointer-slider::-webkit-slider-runnable-track) {
    @apply bg-gray-400 dark:bg-gray-600 rounded-full h-2;
  }

  :global(.pointer-slider::-webkit-slider-thumb) {
    @apply appearance-none w-6 h-6 bg-white rounded-full shadow-md cursor-pointer relative -mt-2;
  }

  :global(.pointer-slider::-moz-range-track) {
    @apply bg-gray-400 dark:bg-gray-600 rounded-full h-2;
  }

  :global(.pointer-slider::-moz-range-thumb) {
    @apply w-6 h-6 bg-white rounded-full shadow-md cursor-pointer border-0;
  }
</style>
