...
<script lang="ts">
  ...
  let weeks: Array<Array<GoalScoreType>> = $state([])
  let months: Array<Array<GoalScoreType>> = $state([])

  let totalDays: number | undefined = $state(undefined)
  let totalWeeks: number | undefined = $state(undefined)
